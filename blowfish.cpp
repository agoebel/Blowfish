 ...
 * Andrew Goebel - atggg3 - July 2012 - CS4780 Cryptography - Instructor: Michael Schulte
 * 
 * Use:
 *		Example:
 *
 *		./blow 0123456789abcdef
 *		Hello World
 *		EOF (Ctrl-D)
 *		4E0727027D06112A
 *
...
/**
 * The class represents 8 bytes encoded in two unsigned integers called Left and
 * Right in the blowfish description.
 */
class Block
{
public:
	/* This constructor reads 8 bytes as two Big Endian unsigned integers */
	Block(const unsigned char *buff) : Left(0), Right(0)
	{
		/* Divide the 8-byte block into two 32-bit integers */
		Left |= *buff++ << 24;
		Left |= *buff++ << 16;
		Left |= *buff++ << 8;
		Left |= *buff++;
	...
	Block(unsigned int s1, unsigned int s2) : Left(s1), Right(s2)
	{
	}
	...
	/* Saves 8 bytes as two Big Endian unsigned integers */
	void Save(unsigned char *buff)
	{
		*buff++ = Left >> 24;
		*buff++ = (Left >> 16) & 0xFF;
		*buff++ = (Left >> 8) & 0xFF;
...
/* Prototypes */
int ReadKey(char *sKey, uint64_t *lKey);
void InitKey(uint64_t lKey);
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2)
	{
		printf("Usage: blowfish key\n");
		return -1;
	}

	uint64_t lKey = 0;
	if (ReadKey(argv[1], &lKey) < 0)
		return -1;
...
/**
 * Parses 16 (and only 16) hexadecimal characters from sKey and puts a 64-bit integer in lKey
...
int ReadKey(char *sKey, uint64_t *lKey)
{
	for (int i = 0; i < 16; i++)
	{
		...
		if (sKey[i] == 0)
		{
			printf("The key is too short. Expected 16 hex chars\n");
			return -1;
		}

		if (!isxdigit(sKey[i]))
		{
			printf("Letter %c is not an hexadecimal char.\n", sKey[i]);
			return -1;
		}

		/* Convert ASCII to int */
		int digit = tolower(sKey[i]);
		if (digit >= '0' && digit <= '9')
			digit -= '0';
		...
		/* Put the new digit in the right place */
		*lKey <<= 4;
		*lKey |= digit;
	}

	/* Longer than 16 chars? Error */
	if (sKey[16] != 0)
	{
		printf("Key is too long. Expected only 16 hexadecimal chars.\n");
		return -1;
	}
...
/**
 * Prepares the P array and S boxes accordingly to the key, given as a 64-bit integer.
 * According to the blowfish algorithm, XOR each entry of P with
 * 32 bits from the key, then if the key is too short, start over.
 */
void InitKey(uint64_t lKey)
{
	/*
	 * Since we always have 64 bits keys, it's faster to alternate between
	 * the most significant and least significant doubleword each time.
	 */
	for (int i = 0; i < 18; i++)
	{
		int k;
	...
	/*
	 * With the P array ready, now encrypt 0x0000000000000000 and save the
	 * results as new values of P, and do the same for all the values of
	 * the four S boxes.
	 */
	Block zero(0, 0);
	for (int i = 0; i < 18;)
	...
	for (int j = 0; j < 4; j++)
	{
		...
		{
			Encrypt(zero);
			SBoxes[j][k] = zero.Left;
			SBoxes[j][k + 1] = zero.Right;
		}
	}
}
...
/* ((S[0] + S[1]) ^ S[2]) + S[3] --- Indexes are taken from the single bytes of val */
...
/**
 * Encrypt using a Block as source AND destination
 */
void Encrypt(Block &b)
{
	/* This is the core of the encryption. Here the 18 rounds took place */

	unsigned int s1 = b.Left;
	unsigned int s2 = b.Right;
...
/**
 * Encrypt using an 8-byte (at least) buffer as source AND destination
 * Please note that @buf is not checked for null or size correctness.
 */
void Encrypt(unsigned char *buf)
{
	/* buf must be at least eight bytes long */
	Block b(buf);
	Encrypt(b);
	b.Save(buf);
}
